...
-- 4. 验证数据是否插入成功
SELECT '=== 月度数据 ===' AS info;
SELECT stat_date, efficiency_score, course_count, student_engagement 
FROM teacher_efficiency_trend 
WHERE teacher_id = 2 AND period_type = 'month' 
ORDER BY stat_date;

SELECT '=== 日度数据 ===' AS info;
SELECT stat_date, efficiency_score, teaching_hours 
FROM teacher_efficiency_trend 
